...
export function AgencyTheme({ profile, links, socials, videos, banners, track }: BioThemeProps) {
  ...
  const accent = profile.button_color || '#BEF264';
  ...
  const marqueeText = MARQUEE_TEXT[s.marqueeWords] || MARQUEE_TEXT.brand;
            ...
            {s.titleFont === 'mix' ? (
              <>
                ...
                <span style={{ fontFamily: sansFamily, fontWeight: 900 }}>
                  {(profile.display_name || profile.username).split(' ').slice(1).join(' ') || ''}
                </span>
              </>
            ) : (
