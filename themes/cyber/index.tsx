...
export function CyberTheme({ profile, links, socials, videos, banners, track }: BioThemeProps) {
  ...
  const accent = profile.button_color || '#22D3EE';
  const promptColor = s.promptColor === 'accent' ? accent : (PROMPT_COLORS[s.promptColor] || '#4ADE80');
      ...
      <div className="relative max-w-md mx-auto px-5 pt-[72px] pb-24 text-sm">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[10px] opacity-70" style={{ color: promptColor }}>USER_PROFILE</div>
            <h1
              className="text-2xl mt-1 truncate cyber-glitch"
              style={{ color: profile.text_color, fontWeight: 700, textShadow: `0 0 8px ${accent}55` }}
              data-text={profile.display_name || `@${profile.username}`}
            >
        ...
        <div className="mt-8 flex items-center gap-2 text-xs" style={{ color: promptColor }}>
          ...
          <span>ls ./links --sort=priority</span>
        </div>
